...
public final class BerlinClockServiceImpl implements BerlinClockService {
	...
	public String convertDigitalToBerlin(final String time) {
		...
		// calculate each row
		final String secondRow = calculateSecondsLampRow(seconds);
		final String fiveHoursRow = calculateFiveHoursRow(hours);
		final String singleHourRow = calculateSingleHoursRow(hours);
		final String fiveMinutesRow = calculateFiveMinutesRow(minutes);
		final String singleMinuteRow = calculateSingleMinutesRow(minutes);
		// join the rows
		return String.join("", secondRow, fiveHoursRow, singleHourRow, fiveMinutesRow, singleMinuteRow);
	}
